...
void Ini::insert_session(const string &session_name) {
  ...
  pair<string, map<string, string>> entry =
      pair<string, map<string, string>>(session_name, session_map);
...
string Ini::get(const string &key, const string &defaultValue,
...
int Ini::insert_entry(map<string, string> *session_map, const string &line) {
  ...
  if (equal_pos == string::npos) {
    cerr << __FILE__ << __FUNCTION__ << "Invalid configuration line " << line
         << endl;
    return -1;
  }
...
void Ini::to_string(string &output_str) {
  ...
  for (SessionsMap::iterator it = sections_.begin(); it != sections_.end();
    ...
    for (map<string, string>::iterator sub_it = section_map.begin();
