...
int writeToFile(const string &fileName, const char *data, uint32_t dataSize,
                const char *openMode);
 ...
 */
int getFileList(vector<string> &fileList, const string &path,
                const string &pattern, bool recursive);
int getFileNum(uint64_t &fileNum, const string &path, const string &pattern,
               bool recursive);
int getDirList(vector<string> &dirList, const string &path,
 ...
 * @param buf 读取到这里
 * @param size 读取的数据长度
 * @return int 返回0表示成功。-1
 * 表示读取到文件尾，并且没有读到size大小数据，其它表示errno
 */
